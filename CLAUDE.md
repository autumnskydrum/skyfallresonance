...
**Environment:** `DATABASE_URL` (Prisma Postgres connection string), `KMA_API_KEY`, `WEATHERAPI_KEY`, `NEWS_CRAWL_SECRET`, `WEATHER_COLLECT_SECRET` all live in `.env` (gitignored) locally, and must be duplicated into the Vercel project's env vars for production — `.env` is never read at runtime on Vercel. `prisma.config.ts` loads `.env` via `dotenv/config` — Prisma does not auto-load `.env` on its own in this version, so don't remove that import.
